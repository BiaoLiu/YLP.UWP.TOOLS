...
        private bool keepContext;
       ...
        /// </summary>
        ...
        {
            HttpWebResponse res = await GetResponse();
        ...
                /// <returns>解码后的字符串</returns>
        public async Task<string> GetString(Encoding encoding)
        {
            byte[] data = await GetBytes();
            return encoding.GetString(data);
        }
        ...
        public async Task<int> HeadContentLength()
        {
            Reset();
            HttpVerb lastVerb = verb;
            verb = HttpVerb.HEAD;
            using (HttpWebResponse res = await GetResponse())
            {
                verb = lastVerb;
